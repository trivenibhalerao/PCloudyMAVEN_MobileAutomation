package com.paybankmobile.maven.com.paybankm.mobile;
...
public class PayBankTestFunction1 {
	...
	public static void main(String[] args) throws IOException, ConnectError, InterruptedException {
		...
		PayBankTestFunction1 britishAirways = new PayBankTestFunction1();
	...
	public void runExecutionOnPCloudy() throws IOException, ConnectError, InterruptedException {
		...
		String authToken = con.authenticateUser("Your Email", "API Key");

		ArrayList<MobileDevice> selectedDevices = new ArrayList();
		
		// selectedDevices.add(MobileDevice.getNew("Samsung_GalaxyE7_Android_5.1.1",
		// 143, "GalaxyE7", "Galaxy E7", "android", "5.1.1", "Samsung"));

		// To select multiple devices manually, use either of these:
		// selectedDevices.addAll(con.chooseMultipleDevices(authToken,
		// "android"));
		// selectedDevices.addAll(CloudyCONNECTOR.chooseSingleDevice(authToken,
		// "android"));
		...
		File fileToBeUploaded = new File("APK PAth.apk");
	...
	private Runnable getTestCaseClass(final URL endpoint, final BookingDtoDevice aDevice,
			final PCloudyAppiumSession pCloudySession, final SingleRunReport report) {
		...
		return new Runnable() {
			...
			public void run() {
				File deviceFolder = new File(ReportsFolder,
						aDevice.manufacturer + " " + aDevice.model + " " + aDevice.version);
				...
				try {
					...
					capabilities.setCapability("appPackage", "altimetrik.android.com.paybank");
					capabilities.setCapability("appActivity",
							"altimetrik.android.com.paybank.view.activity.LoginActivity");
					...
					report.addComment("--- Your Test Scripts over here ---");

					// Automation Scipr here
					...
					HtmlFilePrinter printer = new HtmlFilePrinter(
							new File(deviceFolder, deviceFolder.getName() + ".html"));
